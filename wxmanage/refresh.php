...
if(isset($_GET["info_nav"])){//是获取信息的操作
	...
	if($_GET["info_nav"]==-1){//获取社团列表
		$arr00 = WXDB::getSomething("*",$info,"order by vistcount desc");
...
//管理员更新社团登录信息
if(isset($_POST['seid']) && isset($_POST['managerpassword_edit']) && 
	isset($_POST['acount_edit'])&&isset($_POST['surepassword_edit'])&&
	isset($_POST['password_edit'])){
	$seid=$_POST['seid'];//将社团编号传进服务端，便于高级管理员修改指定社团的资料
	$managerpassword=$_POST['managerpassword_edit'];//管理员密码
	$username=$_POST['acount_edit'];
	$password=$_POST['password_edit'];
	$surepassword=$_POST['surepassword_edit'];
	$mangerpassworddb = WXDB::getSomething("password",$user,"where seid='0'");//高级管理员密码
	$mangerpassworddb = $mangerpassworddb[0][0];//获取密码
	if($mangerpassworddb==$managerpassword){//判断管理员密码输入是否正确
		$update="username='{$username}',password='{$password}'";
		$condition="where seid='{$seid}'";
		$success=WXDB::updateSomething($update,$user,$condition);
		if($success) echo json_encode("updatesuccess"); //密码修改成功返回success字符串
		else echo json_encode("fail");
	}
	else
		echo json_encode("mangerpassworderror");
}
...
if(isset($_POST['changetype'])){
	...
	if($changetype=='changeinfo'){
		...
		if($seid!=""){//有seid的操作是修改社团账号信息
			...
			$user_seid = WXDB::getSomething("seid",$info,$condition);
			$user_seid=$user_seid[0][0];
			$values="'{$user_seid}','{$username}','{$password}'";
			$success2 = WXDB::addOne($user, "seid,username,password", $values);//添加账号
...
if(isset($_POST['opsearch'])){
	...
	if($_POST['opsearch']=="marketsearch")//物资搜索
		...
		$arr = WXDB::getSomething("seid,name",$info,"where name like '{$keyword}'");
	echo json_encode($arr);//返回所有符合搜索条件的社团编号
}

if(isset($_POST['initall'])){//点击所有社团信息时，初始化社团信息
	$arr = WXDB::getSomething("seid,name",$info," order by seid asc limit 5");//取出前4个社团
	echo json_encode($arr);
}

//修改、添加社团账号信息
if(isset($_POST['seid'])&&isset($_POST['name_edit'])&&
	isset($_POST['short_edit'])&&isset($_POST['logo_url'])&&
	isset($_POST['type_edit'])&&isset($_POST['username_edit'])&&isset($_POST['password_edit'])){
	$seid=$_POST['seid'];
	$name=$_POST['name_edit'];
	$short=$_POST['short_edit'];
	$img=$_POST['logo_url'];
	...
	if($seid!=""){//有seid的操作是修改社团账号信息
		...
		if($img!="inexistence"){//修改了图片的链接
			$oldimg=WXDB::getSomething("img",$info,$condition);
			$oldimg=$oldimg[0][0];//图片1的原连接
			if(!empty($oldimg))//存在原链接
				unlink($oldimg);//删除原链接的文件
			$update="{$update},img='{$img}'";//在数据库中修改图片连接
		}
		$success= WXDB::updateSomething($update, $info, $condition);
		if($success)
